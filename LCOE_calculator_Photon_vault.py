...
from hybrid_tea_model_photon_ import hybrid_tea_model_photon
import warnings
...
################################################################# Retrieve TMY data ###################################################################
# Function: retrieves typical meteorological data for a given location/year from NREL NSRDB 
# Inputs: plant location as a string, API key for NREL API, T/F for interpolation 
# Outputs: TMY data, # of time steps, time unit for plotting, latitude, longitude, zenith angle, ambient temperature, time zone, period 
...
# In[ ]:
################################################################# Calculate PV Output #################################################################### 
# Function: calculates hourly PV array output 
# Inputs: latitude, longitude, time zone, TMY data, PV AC rating, tilt angle
# Outputs: hourly PV output, DNI data, TMY data
class pv_system:
    ...
    def run_pv_simulation(self, tmy_data,PV_AC_Rating,tilt):
        ...
        #print(f'PV output: {round(np.sum(hourly_pv_output),2)}')
        return hourly_pv_output, dni_data, tmy_data_


# In[ ]:
################################################################# Calculate CSP Output #################################################################### 
# Function: calculates hourly CSP array output 
# Inputs: latitude, longitude, time zone, TMY data, DNI data, ambient temperature, collector area, tilt angle
# Outputs: hourly CSP output
class csp_system:
    ...
    def csp_performance_simulation(self, tmy_data_, dni_data, amb_temp, collector_area, tilt):
        ...
        #print(f'CSP output: {round(np.sum(csp_thermal_power),2)}')
...
# Function: calculates TES Dispatch Logic 
# Inputs: hourly PV output, CSP output, TES capacity, maximum hourly TES output, gridtied
# Outputs: load met percentage, total RES power to load, load, yearly TES stored, CSP generation, TES to load, PV to load, grid power required, excess PV, grid contribution to ice annual, PV excess contribution to ice annual, total energy req for ice, curtailed PV
class tes_system:
    def __init__(self, period, no_ts, load, amb_temp):
        ...
        self.curtailed_pv_excess = np.zeros(period)
    ...
    def energybalance(self, hourly_pv_output, csp_thermal_power, tes_capacity, maximum_hourly_tes_output, gridtied):
        ...
        # scenario 2: isolated, updated TES logic (excess PV to 1/4 elec, 3/4 ice making)
            ...
            if gridtied == False: 
                ...
                tes_stored += possible_charge * self.tes_charge_efficiency
                self.curtailed_pv_excess[i] = charge_from_pv_excess - possible_charge
        ...
        curtailed_pv = self.curtailed_pv_excess
...
# In[ ]:
################################################################# Calculate LCOE #################################################################### 
# Function: calculate LCOE 
# Inputs: hourly PV output, CSP output, TES capacity, maximum hourly TES output, scenario
# Outputs: LCOE, O&M costs, CAPEX breakdown, annualized costs, NPV
class TEA_Calculations:
    ...
    def calculate_LCOE_OM(self, pv_to_load, tes_to_load, grid_power_required, gridtied, grid_contribution_to_ice_annual):
        ...
        return LCOE, PV_cc, TES_cc, PB_cc, CSP_cc, C_OM_TES, C_OM_PV, C_OM_CSP, C_OM_PB, CAPEX, NPV, totalgridcost


# In[ ]:
################################################################# Optimize Subcomponent Capacities #################################################################### 
# Function: calculates TES Dispatch Logic 
# Inputs: load, plant location, min LMP, scenario, number of optimization calls
# Outputs: optimal number of collectors, TES capacity, max hourly TES output, tilt angle, PV capacity
def optimize_system(load, plant_location, minimum_load_met_percentage, gridtied, n_calls):
    ...
    def objective_function(params, load=load, tmy_data=tmy_data, no_ts=no_ts, latitude=latitude, longitude=longitude, 
        ...
        LCOE, PV_cc, TES_cc, PB_cc, CSP_cc, C_OM_TES, C_OM_PV, C_OM_CSP, C_OM_PB, CAPEX, NPV, totalgridcost = tea_calc.calculate_LCOE_OM(pv_to_load, tes_to_load, grid_power_required, gridtied, grid_contribution_to_ice_annual)
    ...
    if not valid_results:
        print("no solution found that meets the load met percent requirement")
        return None
...
def get_optimized_results(optimal_params, plant_location, load, gridtied):
    ...
    LCOE, PV_cc, TES_cc, PB_cc, CSP_cc, C_OM_TES, C_OM_PV, C_OM_CSP, C_OM_PB, CAPEX, NPV, totalgridcost = tea_calc.calculate_LCOE_OM(pv_to_load, tes_to_load, grid_power_required, gridtied, grid_contribution_to_ice_annual)
    print(LCOE, PV_cc, TES_cc, PB_cc, CSP_cc, C_OM_TES, C_OM_PV, C_OM_CSP, C_OM_PB, CAPEX, NPV, totalgridcost)
    print(f'LCOE: {LCOE}')
...
#### Performance Plots
def annual_performance_plots(axes, gridtied, days, startday, no_ts, timeunit, amb_temp, period, hourly_pv_output, dni_data, csp_thermal_power, load_met_percentage, total_RES_power_to_load, load, yearly_tes_stored, CSP_gen, tes_to_load, pv_to_load, grid_power_required, excess_pv, grid_contribution_to_ice_annual, pv_excess_contribution_to_ice_annual, total_energy_req_for_ice):
    display = 24*days*no_ts + startday*24 +6 
    hrs = np.arange(0, period, 1)
    starthr = startday*24 +6

    # Energy generation from PV and CSP  
    axes[0].plot(hrs[starthr:display], CSP_gen[starthr:display], label='CSP Generation (kW)', color='blue')
    axes[0].plot(hrs[starthr:display], hourly_pv_output[starthr:display], label='PV Generation (kW)', color='green')
    axes[0].plot(hrs[starthr:display], dni_data[starthr:display], label='DNI (kW)', color='purple')
    axes[0].set_xlabel(timeunit)
    axes[0].set_ylabel('Power (kW)')
    axes[0].set_title('Energy Generation')
    axes[0].legend(loc='upper left')

    # Energy balance - what is going to load 
    axes[1].fill_between(hrs[starthr:display], 0, tes_to_load[starthr:display], color='tomato', alpha=0.7, label='TES to Load')
    axes[1].fill_between(hrs[starthr:display], tes_to_load[starthr:display], tes_to_load[starthr:display] + pv_to_load[starthr:display], color='goldenrod', alpha=0.7, label='PV to Load')
    if gridtied:
        axes[1].fill_between(hrs[starthr:display], total_RES_power_to_load[starthr:display], total_RES_power_to_load[starthr:display] + grid_power_required[starthr:display], color='gray', alpha=0.3, label='Grid to Load (kW)')
    else: 
        axes[1].fill_between(hrs[starthr:display], total_RES_power_to_load[starthr:display], total_RES_power_to_load[starthr:display] + grid_power_required[starthr:display], color='gray', alpha=0.3, label='Unmet Load (kW)')
    axes[1].plot(hrs[starthr:display], load[starthr:display], label='Load (kW)', color='black', linewidth=2)
    axes[1].set_xlabel(timeunit)
    axes[1].set_ylabel('Power (kW)')
    axes[1].set_title('Energy Supplied to the Load')
    axes[1].legend(loc='upper left')

    # Ice production
    print(np.sum(grid_contribution_to_ice_annual))
    axes[2].fill_between(hrs[starthr:display], 0, excess_pv[starthr:display], color='purple', alpha=0.3, label='PV excess (kW)')
    axes[2].fill_between(hrs[starthr:display], 0, pv_excess_contribution_to_ice_annual[starthr:display], color='orange', alpha=1, label='PV excess going to ice production (kW)')
    axes[2].fill_between(hrs[starthr:display], pv_excess_contribution_to_ice_annual[starthr:display], pv_excess_contribution_to_ice_annual[starthr:display] + grid_contribution_to_ice_annual[starthr:display], color='yellow', alpha=1, label='Grid going to ice production (kW)')
    axes[2].plot(hrs[starthr:display], total_energy_req_for_ice[starthr:display], label='Ice Production Requirement (kW)', color='black', linewidth=2)
    axes[2].set_xlabel(timeunit)
    axes[2].set_ylabel('Power (kW)')
    axes[2].set_title('Energy Supplied to Ice Production')
    axes[2].legend(loc='upper left')

    # Energy stored in TES
    axes[3].fill_between(hrs[starthr:display], 0, yearly_tes_stored[starthr:display], color='green', alpha=0.4)
    axes[3].set_xlabel(timeunit)
    axes[3].set_ylabel('Power (kW)')
    axes[3].set_title('Stored Energy')
...
def parametric_plots(load, plant_location, gridtied, n_data_points, collector_area_max, tes_capacity_max, pv_capacity_max, csv_name):

    collector_area_min, collector_area_max = 0, collector_area_max
    tes_capacity_min, tes_capacity_max = 1e4, tes_capacity_max
    pv_capacity_min, pv_capacity_max = 0, pv_capacity_max
    ...
    def run_simulation(params, tmy_data, no_ts, timeunit, latitude, longitude, theta_z, amb_temp, tz, period, load):
        ...
        return load_met_percentage, LCOE, PV_cc, TES_cc, PB_cc, CSP_cc, C_OM_TES, C_OM_PV, C_OM_CSP, C_OM_PB, CAPEX, NPV, total_RES_power_to_load, CSP_gen, hourly_pv_output, curtailed_pv, totalgridcost, yearly_tes_stored, total_energy_req_for_ice, grid_contribution_to_ice_annual,pv_excess_contribution_to_ice_annual
    ...
    def generate_and_plot_results(tmy_data, no_ts, timeunit, latitude, longitude, theta_z, amb_temp, tz, period, load, collector_area_array, tes_capacity_array, pv_capacity_array, max_hourly_tes_output_array):
        ...
        for collector_area, tes_capacity, pv_capacity, maximum_hourly_tes_output in product(collector_area_array, tes_capacity_array, pv_capacity_array,max_hourly_tes_output_array):
            ...
            load_met_percentage, LCOE, PV_cc, TES_cc, PB_cc, CSP_cc, C_OM_TES, C_OM_PV, C_OM_CSP, C_OM_PB, CAPEX, NPV, total_RES_power_to_load, CSP_gen, hourly_pv_output, curtailed_pv, totalgridcost, yearly_tes_stored, total_energy_req_for_ice, grid_contribution_to_ice_annual,pv_excess_contribution_to_ice_annual = run_simulation(params, tmy_data, no_ts, timeunit, latitude, longitude, theta_z, amb_temp, tz, period, load)
            
            results_list.append({
                'LCOE': LCOE,
                'Load Met %': load_met_percentage,
                'Collector Area [m2]': collector_area,
                'TES Cap [kWhth]': tes_capacity,
                'PV Cap [W]': pv_capacity,
                'Max Hourly TES output [kWh]' : maximum_hourly_tes_output,
                'Total CSP output [kW]' : np.sum(CSP_gen),
                'Total PV output [kW]' : np.sum(hourly_pv_output),
                'Total RES power to load [kW]' : np.sum(total_RES_power_to_load),
                'Total TES stored [kWhth]' : np.sum(yearly_tes_stored),
                'Total PV curtailed [kWh]' : np.sum(curtailed_pv),
                'Total grid cost [$]' : totalgridcost,
                'Total Energy Required for Ice [kWh]' : np.sum(total_energy_req_for_ice),
                'Total Grid to Ice [kWh]' : np.sum(grid_contribution_to_ice_annual),
                'Total PV to Ice [kWh] ' : np.sum(pv_excess_contribution_to_ice_annual),
                'Average annual ambient temperature' : np.mean(amb_temp),
                'PV_cc' : PV_cc,
                'TES_cc' : TES_cc,
                'PB_cc' : PB_cc,
                'CSP_cc' : CSP_cc,
        ...
        results_df = results_df.dropna(subset=['Load Met %', 'LCOE'])

        min_lcoe_idx = results_df.groupby('Load Met %')['LCOE'].idxmin()
    ...
    if gridtied:
        ...
        trace_true = go.Scatter(
            x=results_df['Load Met %'],
            y=results_df['LCOE'],
            mode='markers',
            marker=dict(color='blue'),
            text=(
                'Collector area (m2): ' + results_df['Collector Area [m2]'].astype(str) + '<br>' +
                'TES Capacity (Whth): ' + results_df['TES Cap [kWhth]'].astype(str) + '<br>' +
                'PV Capacity  (W): ' + results_df['PV Cap [W]'].astype(str) + '<br>' +
                'Max hourly TES output (Wh): ' + results_df['Max Hourly TES output [kWh]'].astype(str) + '<br>' +
                'LCOE: ' + results_df['LCOE'].astype(str) + '<br>' +
                'Load Met Percentage: ' + results_df['Load Met %'].astype(str)
            ),
        hoverinfo='text'
        )
...
def plot_performance(axes, load, plant_location, gridtied, collector_area, tes_capacity, pv_capacity, maximum_hourly_tes_output, days, startday):
    solar_data_fetcher_ins = solar_data_fetcher(plant_location=plant_location, interpolating=False)
    tmy_data, no_ts, timeunit, latitude, longitude, theta_z, amb_temp, tz, period = solar_data_fetcher_ins.fetch_solar_data()
    tilt = latitude
    ...
    load_met_percentage, total_RES_power_to_load, load, yearly_tes_stored, CSP_gen, tes_to_load, pv_to_load, grid_power_required, excess_pv, grid_contribution_to_ice_annual, pv_excess_contribution_to_ice_annual, total_energy_req_for_ice, curtailed_pv = tes_sys.energybalance(
        hourly_pv_output, csp_thermal_power, tes_capacity, maximum_hourly_tes_output, gridtied)
    print(f'grid contribution to ice {np.sum(grid_contribution_to_ice_annual)}')
    # Annual performance plots
    annual_performance_plots(axes, gridtied, days, startday, no_ts, timeunit, amb_temp, period, hourly_pv_output, dni_data, csp_thermal_power, load_met_percentage, total_RES_power_to_load, load, yearly_tes_stored, CSP_gen, tes_to_load, pv_to_load, grid_power_required, excess_pv, grid_contribution_to_ice_annual, pv_excess_contribution_to_ice_annual, total_energy_req_for_ice)
...
def generate_and_plot_tornado_sensitivities(load, load_met_percentage, tes_to_load, pv_to_load, grid_power_required, grid_contribution_to_ice_annual, gridtied, optimal_params):
    ...
    base_case_LCOE, PV_cc, TES_cc, PB_cc, CSP_cc, C_OM_TES, C_OM_PV, C_OM_CSP, C_OM_PB, CAPEX, NPV, totalgridcost = base_case_tea_calc.calculate_LCOE_OM(pv_to_load, tes_to_load, grid_power_required, gridtied, grid_contribution_to_ice_annual)
